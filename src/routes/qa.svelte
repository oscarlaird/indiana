<script>
    ...
    async function ask_gpt(query) {
        ...
        const contexts = top_k_contexts(vector, 4);
...
{#if sentences}
    <div id="reference-cards">
        {#each sentences.reduce((acc, s) => acc.map(x => x.context_idx).includes(s.context_idx) && ~s.timestamp ? acc : [...acc, s], []) as s}
            <ContextCard sentence={s} />
        {/each}
    </div>
{/if}
